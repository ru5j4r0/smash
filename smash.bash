...
cd ~/.local/share/||(echo -e '\033[0;31m [Error] Failed to load smash.\033[0;0m'&&exit 1)
if test -d smash; then
cd smash||(echo -e '\033[0;31m [Error] Failed to load smash.\033[0;0m'&&exit 1)
if test "$(date +%w)" != "$(cat .smash_last_updated)"; then
date +%w>.smash_last_updated
git pull -q
fi
else
git clone -q https://github.com/ru5j4r0/smash.git
cd smash||(echo -e '\033[0;31m [Error] Failed to load smash.\033[0;0m'&&exit 1)
date +%w>.smash_last_updated
chmod 744 bin/*
